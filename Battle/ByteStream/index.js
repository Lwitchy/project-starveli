...
class ByteStream {
    constructor(data) {
        if (Buffer.isBuffer(data)) {
            ...
            this.buffer = Buffer.allocUnsafe(Math.max(0, data))
        } else {
    ...
    reset(data) {
        if (Buffer.isBuffer(data)) {
            this.buffer = data
        } else if (typeof data === 'number' && this.buffer.length < data) {
    ...
    send() {
        ...
        const header = this.writeHeader(this.id, payload.length, this.version)
